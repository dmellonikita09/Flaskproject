...
def save_picture(form_picture):
    ...
    return picture_fn
def save_uploadpicture(post_picture):
    random_hex = secrets.token_hex(8)
    _, f_ext = os.path.splitext(post_picture.filename)
    post_fn = random_hex + f_ext
    post_path = os.path.join(app.root_path, 'static/upload', post_fn) 
...
def account():
    ...
    if form.validate_on_submit():
        if form.pictures.data:
            picture_file = save_picture(form.pictures.data)
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        #     photo_file = save_uploadpicture(form.upload_picture.data)
        #     upload_file = photo_file
        post = Post(title=form.title.data, content=form.content.data, author=current_user) # upload_file = form.upload_picture.data)
        db.session.add(post)
        db.session.commit()
        flash('Your post has been created!', 'success')
        return redirect(url_for('home'))
   # upload_file = url_for('static', filename='upload/')
    return render_template('create_post.html', title='New Post', form=form, legend='New Post')# upload_file = upload_file)
